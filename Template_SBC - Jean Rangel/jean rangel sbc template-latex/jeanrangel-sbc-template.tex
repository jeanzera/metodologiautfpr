...
Além das técnicas para realizar mineração de dados, técnicas destinadas à visualização e estatística dos dados também foram comentadas pelos autores. Os algoritmos não são destinados a realizar cálculos para transformar os dados. O objetivo das técnicas de visualização e criação de estatística das instâncias é somente apresentar para o usuário uma forma de melhor compreensão dos dados. Em poucos casos, foram utilizadas análises de regressão.
...
\cite{Allan2010} também indicam que é possível o cruzamento de técnicas, onde podemos executar métodos semi-supervisionados somente com informações de dados não fraudulentos, métodos supervisionados sendo aplicados em sequência e métodos supervisionados e não-supervisionados sendo utilizados em conjunto de maneira híbrida.
...
A Figura \ref{anomalias}a apresenta o indivíduo anormal mais recorrente, que uma única instância que está com dados divergentes da maioria das outras instâncias, as quais formaram um \emph{cluster} (agrupamento). Para o especialista em detecção de fraudes, é uma tarefa relativamente simples tratar esse indivíduo.

Na Figura \ref{anomalias}b, há outras instâncias similares com a instância anormal, construindo um novo agrupamento de \emph{outliers}. Caso o grupo seja relativamente grande em comparação com o conjunto normal de instâncias, o profissional responsável por detecção de anomalias pode considerar aquele um novo agrupamento, e então visualizar os dados com dois \emph{clusters}.

Para considerar um grupo diferente do agrupamento geral, a distância entre os elementos nos grupos também é considerada, sendo chamada de densidade. A diferença de densidade pode ser visualizada na Figura \ref{anomalias}c, onde um grupo está próximo de outro, porém cada instância do grupo menor está mais próxima do centro.
...
Os autores \cite{Ahmed2015} indicam a possibilidade de utilizar um conjunto de dados para identificar limites classificadores, porém, é necessário eliminar manualmente os \emph{outliers} do conjunto. Como pode ser visualizado na Figura \ref{anomalias}e, o padrão de identificação é criado a partir dos dados atuais, e então pode ser aplicado em novos dados. Todas as instâncias que estiverem fora dos limites (em vermelho) serão consideradas anomalias. Caso um dado esteja presente entre os dois agrupamentos, conforme a representação da figura, ele será considerado normal, pois o grupo menor não foi removido para a criação do padrão.
...
Os dados representados na Figura \ref{fig:dadosnormaisefraudulentos} são de um contexto real, originários de compras virtuais realizadas por clientes em um site da internet.
